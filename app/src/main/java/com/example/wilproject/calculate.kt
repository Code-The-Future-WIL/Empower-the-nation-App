...
class calculate : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initializeViews()
        setupClickListeners()
        calculateAndDisplayTotal() // Initialize with R0
    }
    ...
    private fun calculateAndDisplayTotal() {
        ...
        if (courseCount == 0) {
            ...
            val displayText = if (discountRate > 0) {
                "R$total - $discountRate% = R$totalAfterDiscount + VAT = R$finalTotal"
            } else {
                "R$total + VAT = R$finalTotal"
            }
